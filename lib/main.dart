...
class AplicativoState extends State<Aplicativo> {
 ...
 Future<void> criarAposta(int betValue, int oddEven, int number) async {
    ...
    var resposta = await http.post(url,
        ...
        body: jsonEncode({
          ...
          'valor': betValue,
          'parimpar': oddEven, // 2 -> par, 1 -> impar
          'numero': number
        }));
 ...
 Future<void> telaPerfil() async {
    ...
    if (response.statusCode == 200 || response.statusCode == 204) {
      ...
      throw const HttpException('Falha para retornar aposta');
    }
  }
  ...
  Future<void> desafiarJogador(String challengedName) async {
    ...
    if (response.statusCode == 200 || response.statusCode == 204) {
      ...
      if (json.containsKey('msg')) {
        resultadoAtual = "Empate! Ninguém vai ganhar pontos...";
      } else {
        var winner = json['vencedor'];
        var loser = json['perdedor'];
        if (winner['username'] == nomeJogador) {
          resultadoAtual = 'Parabéns! Você ganhou ${loser['valor']} pontos!';
        } else {
          resultadoAtual = 'Que pena, Você perdeu ${winner['valor']} pontos!';
        }
      }
